...
export const AICommandCenterPanel = ({
  ...
  const { messages, sendMessage, isLoading } = useAICommandCenter();
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-[480px] p-0 border-l border-border/50 bg-gradient-to-br from-background/95 via-background/98 to-background backdrop-blur-xl"
      >
            ...
            <div className="grid gap-2">
              {quickActions.map((action, idx) => (
                <Button
                  ...
                  variant="outline"
                  className={cn(
                    "justify-start h-auto py-3 px-4 bg-gradient-to-r hover:opacity-90 transition-opacity border-0 text-white",
                    action.gradient
                  )}
